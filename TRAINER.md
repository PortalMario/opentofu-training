...
# Group Info
In groups larger than 1, each trainee needs different container names and 2 nginx docker tags each.
...
- Using variables
- Sensitive variables
- Difference between destroy-create change and in-place change.
...
- Previous lecture can be copied but all variables need to wiped and note that docker_container resource needs a value called image.
- Explain iteration limitations (only maps).
- Show json format tfvars and ymal via yamlload function.
...
- Outputs in modules.
- **Outputs from a module to the root module—use `id` as an example output! (Variables and iteration must be removed for this. Everyone should remove them together with the trainer.)**
